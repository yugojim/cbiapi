...
def find_diagnosis(data):
    ...
    # 在 variant_report 中查找
    final_report_diagnosis = data["ResultsReport"]["ResultsPayload"].get("FinalReport", {}).get("PMI", {}).get("SubmittedDiagnosis")
...
def save_sample_data(data_sample_df, output_folder, output_file_data_clinical_sample):
    ...
    with open(output_file_path, 'w', encoding="utf-8", newline='\r\n') as f:
        # 添加四行以#開頭的註解
        f.write(f'#Patient Identifier{sep}Sample Identifier{sep}Path Number{sep}MP Number{sep}Cell Block Number{sep}Cancer Type{sep}Cancer Type Detailed{sep}Tumor Purity{sep}Report Assay{sep}Report Date\n')
        f.write(f'#Identifier to uniquely specify a patient.{sep}A unique sample identifier.{sep}A patient identifier.{sep}A report identifier.{sep}A cell block number.{sep}Cancer type.{sep}Cancer type detailed.{sep}Proportion of tumor cells within a tissue sample.{sep}Report Assay.{sep}Report Date.\n')
        f.write(f'#STRING{sep}STRING{sep}STRING{sep}STRING{sep}STRING{sep}STRING{sep}STRING{sep}NUMBER{sep}STRING{sep}STRING\n')
        f.write(f'#1{sep}1{sep}1{sep}1{sep}1{sep}1{sep}1{sep}1{sep}1{sep}1\n')
        # 儲存 .txt 文件
...
def save_mutations_data(data_mutations_df, output_folder, output_file_data_mutations_extended):
    # # 空白值、遺失值填上 'NA'
    # data_mutations_df = data_mutations_df.fillna('NA', inplace=True)
    # 空白值、遺失值填上 'NA'
...
data_sample_list = ["PATIENT_ID", "SAMPLE_ID", "PATH_NUMBER", "MP_NUMBER", "CELL_BLOCK_NUMBER", "CANCER_TYPE", "CANCER_TYPE_DETAILED", "TUMOR_PURITY", "REPORT_ASSAY", "REPORT_DATE"]
data_patient_list = ["PATIENT_ID", "DIAGNOSIS", "STAGE", "AGE", "SEX", "ETHNICITY"]
...
for i in range(len(all_json_data)):
    ...
    new_sample_data = {
        ...
        "CANCER_TYPE": find_cancer_category((lambda data: find_diagnosis(data) if (result := find_cancer_type(data)) is None else result)(data), common_cancer_type_dict),
        "CANCER_TYPE_DETAILED": (lambda data: find_diagnosis(data) if (result := find_cancer_type_detailed(data)) is None else result)(data),
        "TUMOR_PURITY": find_percent_tumor_nuclei(data),
